export const MEMORY_FRAGMENTS = [
  ...
  export const STORY_EVENTS = [
    ...
    // === ДЕНЬ 30: БРОДЯГА #1 — ПЕРВАЯ ВСТРЕЧА ===
    {
      ...
      choices: [
        { text_ru: '«Я СЛЕДУЮЩИЙ?»', text_en: '"AM I NEXT?"', action: (Events, S, t) => { S.get().player.humanity = Math.max(0, S.get().player.humanity - 5); Events.emit('ui:toast', t('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«ЕСЛИ БУДЕШЬ НЕОСТОРОЖЕН — ДА. ВОЗЬМИ ЭТО, МНЕ НЕ НУЖНО». Вы получили боеприпасы.', text_en: '"IF YOU ARE CARELESS — YES. TAKE THIS, I DON\'T NEED IT." You received ammunition.', choices: [{ text_ru: 'ВЗЯТЬ', text_en: 'TAKE', action: (Events, S, t) => { Events.emit('game:applyReward', { ammo: 5 }); S.get().flags.drifterMet = 1; } }] }); Events.emit('ui:renderMain'); } },
        { text_ru: '«МНЕ ВСЁ РАВНО»', text_en: '"I DON\'T CARE"', action: (Events, S, t) => { S.get().player.humanity = Math.min(100, S.get().player.humanity + 5); Events.emit('ui:toast', t('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«ХОРОШАЯ ПОЗИЦИЯ ДЛЯ ЭТОГО МИРА. УДАЧИ.» Бродяга уходит, не оглядываясь.', text_en: '"GOOD STANCE FOR THIS WORLD. GOOD LUCK." The Drifter leaves without looking back.' }); S.get().flags.drifterMet = 1; Events.emit('ui:renderMain'); } }
      ]
    },
    ...
    // === ДЕНЬ 110: БРОДЯГА #5 — ПРЕДЛОЖЕНИЕ ===
    {
      ...
      choices: [
        { text_ru: 'ДОГОВОРИТЬСЯ', text_en: 'AGREE', action: (Events, S, t) => { S.get().player.humanity = Math.max(0, S.get().player.humanity - 5); Events.emit('ui:toast', t('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«По рукам. Слушай — если попадёшь в беду в секторе 7, ищи люк с красной меткой. Это мой запасной выход». Вы получили карту.', text_en: '"Deal. Listen — if you get into trouble in Sector 7, look for a hatch with a red mark. It\'s my emergency exit." You received a map.', choices: [{ text_ru: 'ПРИНЯТЬ', text_en: 'TAKE', action: (Events, S, t) => Events.emit('game:applyReward', { food: 3, water: 3 }) }] }); Events.emit('ui:renderMain'); } },
        { text_ru: 'ОТКАЗАТЬ', text_en: 'REFUSE', action: (Events, S, t) => { S.get().player.humanity = Math.min(100, S.get().player.humanity + 5); Events.emit('ui:toast', t('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Ладно. Твоё право. Но предложение остаётся в силе.»', text_en: '"Fine. Your right. But the offer stays open."' }); Events.emit('ui:renderMain'); } }
      ]
    },
