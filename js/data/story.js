...
export const STORY_EVENTS = [
  ...
  // === ДЕНЬ 15: ПЕРВЫЙ ПЕРЕХВАТ ===
  {
    ...
    choices: [
      { text_ru: '«Я И ЕСТЬ ЧЕЛОВЕК»', text_en: '"I AM HUMAN"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'СИСТЕМА', speaker_en: 'SYSTEM', img: 'img/portrait_sys.webp', text_ru: 'ВАШИ ЭМОЦИИ ЗАФИКСИРОВАНЫ. УРОВЕНЬ САМОИДЕНТИФИКАЦИИ: 87%. КОРРЕКЦИЯ ПОВЕДЕНИЯ НЕ ТРЕБУЕТСЯ. ПОКА.', text_en: 'YOUR EMOTIONS RECORDED. SELF-IDENTIFICATION LEVEL: 87%. BEHAVIORAL CORRECTION NOT REQUIRED. FOR NOW.' }); Events.emit('ui:renderMain'); } },
      { text_ru: '«КТО ТАКОЙ ОБЪЕКТ 73?»', text_en: '"WHO IS OBJECT 73?"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'АРХИВ', speaker_en: 'ARCHIVE', img: 'img/portrait_archive.webp', text_ru: 'ДАННЫЕ ЗАСЕКРЕЧЕНЫ ДО ЦИКЛА 30. ПРОДОЛЖАЙТЕ ДВИЖЕНИЕ.', text_en: 'DATA CLASSIFIED UNTIL CYCLE 30. CONTINUE MOVEMENT.' }); Events.emit('ui:renderMain'); } }
    ]
  },
  ...
  // === ДЕНЬ 30: БРОДЯГА #1 — ПЕРВАЯ ВСТРЕЧА ===
  {
    ...
    choices: [
      { text_ru: '«Я СЛЕДУЮЩИЙ?»', text_en: '"AM I NEXT?"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«ЕСЛИ БУДЕШЬ НЕОСТОРОЖЕН — ДА. ВОЗЬМИ ЭТО, МНЕ НЕ НУЖНО». Вы получили боеприпасы.', text_en: '"IF YOU ARE CARELESS — YES. TAKE THIS, I DON\'T NEED IT." You received ammunition.', choices: [{ text_ru: 'ВЗЯТЬ', text_en: 'TAKE', action: (Events, GameState, translate) => { Events.emit('game:applyReward', { ammo: 5 }); GameState.get().flags.drifterMet = 1; } }] }); Events.emit('ui:renderMain'); } },
      { text_ru: '«МНЕ ВСЁ РАВНО»', text_en: '"I DON\'T CARE"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«ХОРОШАЯ ПОЗИЦИЯ ДЛЯ ЭТОГО МИРА. УДАЧИ.» Бродяга уходит, не оглядываясь.', text_en: '"GOOD STANCE FOR THIS WORLD. GOOD LUCK." The Drifter leaves without looking back.' }); GameState.get().flags.drifterMet = 1; Events.emit('ui:renderMain'); } }
    ]
  },
  ...
  // === ДЕНЬ 50: БРОДЯГА #2 — ОН БЫЛ ОХРАННИКОМ ===
  {
    ...
    choices: [
      { text_ru: '«Расскажи про Иерихон»', text_en: '"Tell me about Jericho"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Сектор 4 — страшное место. А вот Сектор 7 — ещё хуже. Там Амазонка базируется. Синтетик-убийца, создана специально против таких как ты. Держись от неё подальше.»', text_en: '"Sector 4 is a terrifying place. But Sector 7 — even worse. That\'s where Amazon is based. A killer synth, created specifically against ones like you. Stay away from her."' }); Events.emit('ui:renderMain'); } },
      { text_ru: '«Почему ты мне помогаешь?»', text_en: '"Why are you helping me?"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Потому что я видел, как они поступают с людьми. И ты... ты напоминаешь мне одного человека. Не знаю почему. Просто напоминаешь.»', text_en: '"Because I saw what they do to people. And you... you remind me of someone. I don\'t know why. You just do."' }); Events.emit('ui:renderMain'); } }
    ]
  },
  {
  ...
  // === ДЕНЬ 80: БРОДЯГА #3 — ОН ЗНАЛ ЯНКОВСКОГО ===
  {
    ...
    choices: [
      { text_ru: '«Наверное. Я не знаю»', text_en: '"Probably. I don\'t know"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Янковский был хорошим человеком. Он не заслужил... этого. Прости, что смотрю на тебя так. Просто... это странно.» Бродяга отворачивается, скрывая лицо.', text_en: '"Yankovsky was a good man. He didn\'t deserve... this. Sorry for staring. Just... it\'s strange." The Drifter turns away, hiding his face.' }); Events.emit('ui:renderMain'); } },
      { text_ru: '«Расскажи мне о Янковском»', text_en: '"Tell me about Yankovsky"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Янковский верил в Иерихон. Работал там. А потом что-то увидел — и попытался бежать. Амазонка его поймала. С тех пор его тело больше не принадлежит ему.» Он умолкает.', text_en: '"Yankovsky believed in Jericho. Worked there. Then he saw something — and tried to run. Amazon caught him. Since then, his body no longer belongs to him." He falls silent.' }); Events.emit('ui:renderMain'); } }
    ]
  },
  ...
  // === ДЕНЬ 105: БРОДЯГА #4 — СНОВА ВСТРЕЧА ===
  {
    ...
    choices: [
      { text_ru: '«Похоже на то»', text_en: '"Seems like it"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Тогда ты — единственный шанс остановить всё это. Я серьёзно. Доберись до главной лаборатории. Там должно быть что-то, что объясняет... зачем всё это».', text_en: '"Then you\'re the only chance to stop all this. I\'m serious. Reach the main lab. There must be something there that explains... what is all this for."' }); Events.emit('ui:renderMain'); } },
      { text_ru: '«Просто везёт»', text_en: '"Just lucky"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Ха. Везение — это тоже навык. Держись, парень.»', text_en: '"Ha. Luck is a skill too. Hang in there, kid."' }); Events.emit('ui:renderMain'); } }
    ]
  },
  ...
  // === ДЕНЬ 110: БРОДЯГА #5 — ПРЕДЛОЖЕНИЕ ===
  {
    ...
    choices: [
      { text_ru: 'ДОГОВОРИТЬСЯ', text_en: 'AGREE', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«По рукам. Слушай — если попадёшь в беду в секторе 7, ищи люк с красной меткой. Это мой запасной выход». Вы получили карту.', text_en: '"Deal. Listen — if you get into trouble in Sector 7, look for a hatch with a red mark. It\'s my emergency exit." You received a map.', choices: [{ text_ru: 'ПРИНЯТЬ', text_en: 'TAKE', action: (Events, GameState, translate) => Events.emit('game:applyReward', { food: 3, water: 3 }) }] }); Events.emit('ui:renderMain'); } },
      { text_ru: 'ОТКАЗАТЬ', text_en: 'REFUSE', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Ладно. Твоё право. Но предложение остаётся в силе.»', text_en: '"Fine. Your right. But the offer stays open."' }); Events.emit('ui:renderMain'); } }
    ]
  },
  ...
  // === ДЕНЬ 190: ПРОЩАНИЕ БРОДЯГИ ===
  {
    ...
    choices: [
      { text_ru: '«До встречи»', text_en: '"See you"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.min(100, GameState.get().player.humanity + 5); Events.emit('ui:toast', translate('toast_humanity', '+5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«...До встречи.» Что-то в его голосе говорит, что он не верит в это. Вы уходите.', text_en: '"...See you." Something in his voice says he doesn\'t believe it. You leave.' }); Events.emit('ui:renderMain'); } },
      { text_ru: '«Идём вместе»', text_en: '"Let\'s go together"', action: (Events, GameState, translate) => { GameState.get().player.humanity = Math.max(0, GameState.get().player.humanity - 5); Events.emit('ui:toast', translate('toast_humanity', '-5')); Events.emit('ui:showDialogue', { speaker: 'БРОДЯГА', speaker_en: 'DRIFTER', img: 'img/portrait_drifter.webp', text_ru: '«Я... нет. Это твой путь. Но я подожду здесь. Если выберешься — расскажи мне, что там было.»', text_en: '"I... no. This is your path. But I\'ll wait here. If you make it out — tell me what was there."' }); Events.emit('ui:renderMain'); } }
    ]
  },
